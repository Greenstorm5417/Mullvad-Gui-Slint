...
Package: mullvad-gui-slint
Version: $deb_version
Section: net
Priority: optional
Architecture: $deb_arch
Maintainer: Mullvad-Gui-Slint contributors
...
dpkg-deb --root-owner-group --build target/package-root \
  "dist/mullvad-gui-slint_${version}_${deb_arch}.deb"

mkdir -p target/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
rpmbuild -bb packaging/rpm/mullvad-gui-slint.spec \
...
tar -C target/package-root/usr -czf \
  "dist/mullvad-gui-slint_${version}_${architecture}.tar.gz" .
